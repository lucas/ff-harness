...
- **Last green test:** `uv run pytest tests/ -m "not live"` (271 passed, 1 deselected — Steps 0-11 + chat-first UI + iter-cap fix + state-alarm auto-resolve + /resume force-continue unstick); `uv run pyright harness/ tests/` (0 errors); `docker build` succeeds (377MB image).
