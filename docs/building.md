...
- **Last green test:** `uv run pytest tests/ -m "not live"` (308 passed, 1 deselected — Steps 0-11 + UI polish: chat-first, markdown, dedup, Other keeps options, approval-card rendering, themed HTML mockup with iframe preview); `uv run pyright harness/ tests/` (0 errors); `docker build` succeeds (377MB image).
