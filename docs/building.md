...
- **Last green test:** `uv run pytest tests/ -m "not live"` (296 passed, 1 deselected — Steps 0-11 + UI polish: chat-first, markdown, dedup, Other keeps options, approval-card rendering for business_brief / mockup); `uv run pyright harness/ tests/` (0 errors); `docker build` succeeds (377MB image).
