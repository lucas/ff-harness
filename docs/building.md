...
- **Last green test:** `uv run pytest tests/ -m "not live"` (323 passed, 1 deselected — Steps 0-11 + UI polish through approval-card + HTML mockup iframe + Rewind to awaiting_human); `uv run pyright harness/ tests/` (0 errors); `docker build` succeeds (377MB image).
