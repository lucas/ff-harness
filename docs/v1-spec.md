...
CREATE TABLE llm_calls (
  ...
  status              TEXT NOT NULL,                                       -- ok|rate_limited|transport_error|repair_retry|parse_error|language_violation
  error_message       TEXT,
...
| `output_schema_violation` | `critical` | `{parse_error, repair_attempt, raw_text_preview}` | "Repair attempted {repair_attempt}x and still invalid. Stop and surface raw_text_preview to human." Also raised on terminal language drift: when both the original response and the English-strict one-shot retry contain a meaningful proportion of CJK chars (absolute count >= 8 OR ratio >= 3% of total chars), `parse_error` is prefixed with `language_violation:` and `repair_attempt=2`. |
...
| `render_mockup` | `{layout_spec: dict}` | `ToolResult(ok=True, result={ascii, regions, html, themed})` (themed iff a `business_brief` material is on file; HTML is a self-contained `<!doctype html>` document for iframe preview — generated by the code worker `qwen/qwen3-coder:free` when `ToolContext.code_chat` is wired in, with a deterministic Python template as the safety-net fallback for tests, transport failures, rate limits, or LLM output that fails the doctype/no-`<script>` sanity validator; rendered as a full-width attachment under the agent bubble, not inside the bubble) | n/a | no |
...
The code worker is also called outside of the `build` stage: `render_mockup` (in the `mockup` stage) invokes it via a `code_chat` closure threaded through `OrchestratorConfig` → `ToolContext` to generate the themed mockup HTML. So the code model serves two roles in v1: `write_file` HTML/CSS authoring in `build`, AND mockup HTML generation in `mockup`. Mockup LLM calls are logged to the per-session `llm_calls` table just like worker chat calls; failures cleanly fall back to the deterministic Python renderer in `services/tools/mockup.py`.
...
`LLMWorker.act()` runs two INDEPENDENT one-shot retries per turn — the 429 swap above and a CJK-content guardrail — so a worst-case turn issues up to FOUR API calls: original + language retry + envelope parse + repair retry. After every successful API attempt (HTTP-wise), BEFORE envelope parse, the worker counts CJK characters in the response (CJK Unified Ideographs U+4E00..U+9FFF, Ext A U+3400..U+4DBF, Hiragana, Katakana, Hangul Syllables). If absolute count ≥ 8 OR ratio ≥ 3% of total chars, the row is re-tagged `language_violation` in `llm_calls` and the worker issues one English-strict retry on the SAME model with a system-role directive ("CRITICAL: Reply in ENGLISH ONLY. …") appended to the messages. On retry success the parse pipeline continues normally; on retry-also-CJK an `output_schema_violation` alarm is raised with `parse_error='language_violation: …'`, `repair_attempt=2`, `raw_text_preview` from the retry response, and the worker returns Escalate. Language-violation calls write a `spend_log` row (the API call DID succeed and was billed) — consistent with how repair_retry / parse_error rows are billed.
...
- `worker_output` → agent bubble. `final` envelopes render the summary with a "final" tag; `escalate` envelopes render the reason with an "escalate" tag; `tool_call` envelopes use per-tool templates — every tool_call bubble has its own semantic renderer; raw JSON is NEVER shown to the user. The renderers: `ask_user` → the question text (markdown); `request_approval` → a subject-aware approval card (see "Approval bubbles" below); `save_business_brief` → the same Business-Brief card layout with a "Saving Business Brief" heading (so the user can distinguish "saving the collected brief into session memory" from "approve this brief"); `render_mockup` → the bubble carries only a small `mockup-card-caption` div (`Mockup preview ({N} sections[, themed])`) and the conversation entry also gets an `attachment_html` slot holding the sandboxed `<iframe class="mockup-preview" sandbox="" srcdoc="..." width="900" height="640">` whose document is the themed HTML from the persisted `mockup` material (looked up via `materials_by_id`); the template renders the iframe inside a `.chat-attachment` div under the bubble at `max-width:900px` so it isn't crushed by the bubble's 70%-viewport cap; when no mockup material is available the bubble falls back to the plain `Rendered mockup ({N} sections)` summary and no attachment is emitted; `write_file` → a `tool-call-card` showing `Wrote <code>{path}</code> ({N} bytes)` with the content tucked into a collapsed `<details>` truncated to the first 500 characters (longer files get a "… (N more characters)" footer) so a 50kB write doesn't blow up the chat panel; `read_file` and `list_files` → one-liner `tool-call-card`s naming the path. Any future tool with no explicit branch falls back to a generic labeled-list card (`_render_generic_tool_call_card`): tool name as a title-cased heading plus a `<dl class="brief-rows">` of args — still no JSON, ever. All user values are `html.escape`d before insertion.
