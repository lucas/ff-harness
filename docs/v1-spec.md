...
- Jinja2 server-rendered web UI (session list, detail, awaiting form, iframe of generated site, 2s polling).
...
harness/
  ...
  templates/                    # Layer 4 — frontend
    ...
    session.html                # event timeline + checkpoints + alarms + cost + site iframe
    awaiting.html               # pending question form
data/
  harness.db                    # core DB
...
1. **Bootstrap.** Session created; orchestrator routes to chat worker. Bootstrap skill is injected via `WorkerContext.system_prompt`. Worker batches `ask_user` calls (~3 rounds: name + industry, contact + location, aesthetic + palette) — each round pauses the session, the user answers via `awaiting.html`, loop resumes. Brief is assembled and persisted as a `business_brief` material.
