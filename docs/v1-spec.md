...
- `worker_output` → agent bubble. `final` envelopes render the summary with a "final" tag; `escalate` envelopes render the reason with an "escalate" tag; `tool_call` envelopes use per-tool templates (`ask_user` → the question text, `request_approval` → a subject-aware approval card (see "Approval bubbles" below), `render_mockup` → the bubble carries only a small `mockup-card-caption` div (`Mockup preview ({N} sections[, themed])`) and the conversation entry also gets an `attachment_html` slot holding the sandboxed `<iframe class="mockup-preview" sandbox="" srcdoc="..." width="900" height="640">` whose document is the themed HTML from the persisted `mockup` material (looked up via `materials_by_id`); the template renders the iframe inside a `.chat-attachment` div under the bubble at `max-width:900px` so it isn't crushed by the bubble's 70%-viewport cap; when no mockup material is available the bubble falls back to the plain `Rendered mockup ({N} sections)` summary and no attachment is emitted; `write_file` / `read_file` / `list_files` → one-line file action summaries).
